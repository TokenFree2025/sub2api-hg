...
| **Stripe** | 银行卡、支付宝、微信支付、Link、Kakao Pay 等 | 国际支付，支持多币种 |
...
Kakao Pay 作为 Stripe 子支付方式配置。启用 `kakao_pay` 时，请使用币种为 `KRW` 的 Stripe 服务商实例。
...
用户选择充值金额和支付方式
  ...
  └─ Stripe     → Payment Element（银行卡/支付宝/微信/Kakao Pay 等）
       │
       ▼
  支付回调验签 → 订单 PAID
       │
