  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.rate_limits) {
      for (const rl of data.rate_limits) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${localizedWindowLabel(rl.window)})`,
          value: valueStr,
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')} (${localizedQuotaPeriod('daily')})`, value: `${usd(sub.daily_usage_usd)} / ${usd(sub.daily_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${localizedQuotaPeriod('weekly')})`, value: `${usd(sub.weekly_usage_usd)} / ${usd(sub.weekly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${localizedQuotaPeriod('monthly')})`, value: `${usd(sub.monthly_usage_usd)} / ${usd(sub.monthly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.expires_at) {
        rows.push({
...
function formatDate(iso: string | null | undefined): string {
  ...
  const loc = normalizedLocale === 'zh' ? 'zh-CN' : normalizedLocale === 'ko' ? 'ko-KR' : 'en-US'
  return d.toLocaleDateString(loc, { year: 'numeric', month: 'long', day: 'numeric' })
}

function localizedWindowLabel(window: string): string {
  const normalizedLocale = String(locale.value || '').toLowerCase()
  if (window === '5h') return '5H'
  if (window === '7d') return '7D'
  if (window === '1d') {
    if (normalizedLocale === 'zh') return '日'
    if (normalizedLocale === 'ko') return '일'
    return 'D'
  }
  return window
}

function localizedQuotaPeriod(kind: 'daily' | 'weekly' | 'monthly'): string {
  const normalizedLocale = String(locale.value || '').toLowerCase()
  if (normalizedLocale === 'zh') {
    return kind === 'daily' ? '日' : kind === 'weekly' ? '周' : '月'
  }
  if (normalizedLocale === 'ko') {
    return kind === 'daily' ? '일' : kind === 'weekly' ? '주' : '월'
  }
