  ...
  useI18n: () => ({
    t: (key: string, params?: Record<string, string>) => {
      ...
      if (key === 'auth.oauthFlow.wechatNativeAppOnly') {
        return '이 WeChat 로그인 흐름은 네이티브 모바일 앱에서만 사용할 수 있습니다.'
      }
      return key
    },
  }),
...
describe('WechatCallbackView', () => {
  ...
  it('shows a Korean unavailable hint when WeChat OAuth requires the native app', async () => {
    routeState.query = {
      wechat_bind_existing: '1',
      redirect: '/profile',
      mode: 'open',
    ...
    expect(wrapper.text()).toContain('이 WeChat 로그인 흐름은 네이티브 모바일 앱에서만 사용할 수 있습니다.')
    expect(replaceMock).not.toHaveBeenCalled()
  })
