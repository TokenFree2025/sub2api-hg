              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "tokendeal") +
                        " 100 " +
                ...
                <!-- Row 4: Enabled payment types (provider badges like tokendealpay) -->
                <div>
                  <label class="input-label">{{
...
import { getDefaultLoginAgreementDocuments } from "@/utils/loginAgreement";
...
const isKoLocale = computed(() => locale.value.startsWith("ko"));

const koreanLocalTextMap: Record<string, string> = {
  "邮箱快捷登录": "이메일 OAuth 로그인",
  "开启 GitHub 或 Google 邮箱授权登录后，系统会读取已验证邮箱，存在则直接登录，不存在则自动注册。": "GitHub 또는 Google 이메일 OAuth 로그인을 활성화하면 시스템이 검증된 이메일을 읽어 기존 사용자는 바로 로그인시키고, 없으면 자동으로 가입시킵니다.",
  "GitHub OAuth App 需要 read:user user:email 权限，回调地址填写下方后端地址。": "GitHub OAuth App에는 read:user 및 user:email 권한이 필요하며, 콜백 주소에는 아래의 백엔드 주소를 입력하세요.",
  "后端回调地址": "백엔드 콜백 URL",
  "生成并复制": "생성 후 복사",
  "前端回跳地址": "프론트엔드 리디렉션 URL",
  "Google OAuth 客户端需要 openid email profile 范围，并在凭据里登记后端回调地址。": "Google OAuth 클라이언트에는 openid, email, profile 범위가 필요하며 자격 증명에 백엔드 콜백 주소를 등록해야 합니다.",
  "开通引导：Google Cloud Console → APIs & Services → OAuth consent screen 完成同意屏幕；Credentials → Create Credentials → OAuth client ID，类型选择 Web application，并把下面地址加入 Authorized redirect URIs。": "설정 가이드: Google Cloud Console → APIs & Services → OAuth consent screen에서 동의 화면을 완료한 뒤, Credentials → Create Credentials → OAuth client ID로 이동해 Web application을 선택하고 아래 주소를 Authorized redirect URIs에 추가하세요.",
  "PC 应用": "PC 앱",
  "桌面浏览器通过微信开放平台扫码登录。可与公众号或移动应用同时存在。": "데스크톱 브라우저는 WeChat Open Platform QR 로그인으로 인증합니다. 공식 계정 또는 모바일 앱과 함께 사용할 수 있습니다.",
  "PC AppID": "PC 앱 ID",
  "PC AppSecret": "PC 앱 시크릿",
  "公众号": "공식 계정",
  "仅在微信内浏览器可用；非微信环境下会显示不可用。": "WeChat 브라우저 내부에서만 사용할 수 있으며, 다른 환경에서는 사용할 수 없는 것으로 표시됩니다.",
  "公众号 AppID": "공식 계정 App ID",
  "公众号 AppSecret": "공식 계정 App Secret",
  "移动应用": "모바일 앱",
  "原生移动端通过微信 SDK 唤起授权，网页端不会直接发起该流程。": "네이티브 모바일 클라이언트는 WeChat SDK로 인증을 시작하며, 웹 UI에서는 이 흐름을 직접 시작하지 않습니다.",
  "移动应用 AppID": "모바일 앱 ID",
  "移动应用 AppSecret": "모바일 앱 시크릿",
  "如果同时启用 PC 应用和公众号/移动应用，这些应用需要挂在同一个微信开放平台主体下，否则 UnionID 无法稳定归并账号。": "PC 앱과 공식 계정/모바일 앱을 동시에 활성화하면 동일한 WeChat Open Platform 주체 아래에 있어야 UnionID로 계정을 안정적으로 병합할 수 있습니다.",
  "浏览器回调地址": "브라우저 콜백 URL",
  "用于 PC 应用和公众号的网页回调。移动应用走原生 SDK 时不直接使用这个浏览器回调。": "PC 앱과 공식 계정의 웹 콜백에 사용됩니다. 모바일 앱의 네이티브 SDK 흐름은 이 브라우저 콜백을 직접 사용하지 않습니다.",
  "登录条款确认": "로그인 약관 동의",
  "控制登录页是否要求用户先阅读并同意服务条款、隐私政策或其他 Markdown 文档。": "로그인 페이지에서 사용자가 서비스 약관, 개인정보 처리방침 또는 기타 Markdown 문서를 먼저 읽고 동의해야 하는지 제어합니다.",
  "已启用": "사용 중",
  "未启用": "사용 안 함",
  "展示形式": "표시 방식",
  "弹窗": "모달",
  "复选框": "체크박스",
  "复选框会显示在登录按钮下方，未勾选前所有登录入口禁用。": "체크박스는 로그인 버튼 아래에 표시되며, 체크하기 전까지 모든 로그인 방식이 비활성화됩니다.",
  "弹窗会在登录页打开，用户拒绝后所有登录入口保持禁用。": "모달은 로그인 페이지에서 열리며, 사용자가 거부하면 모든 로그인 방식이 계속 비활성화됩니다.",
  "条款更新日期": "약관 업데이트 날짜",
  "日期或文档内容变化后，用户需要重新同意。": "날짜나 문서 내용이 바뀌면 사용자는 다시 동의해야 합니다.",
  "协议文档": "약관 문서",
  "文档名称可自定义，内容按 Markdown 保存。可参考：服务条款、使用政策、支持的国家和地区、服务特定条款。": "문서 제목은 자유롭게 지정할 수 있으며, 내용은 Markdown으로 저장됩니다. 예: 서비스 약관, 이용 정책, 지원 국가 및 지역, 서비스별 약관.",
  "添加文档": "문서 추가",
  "未命名文档": "제목 없는 문서",
  "文档名称": "문서 제목",
  "路由标识": "라우트 슬러그",
  "Markdown 内容": "Markdown 내용",
  "通过 GitHub 已验证邮箱首次注册或首次绑定时应用。": "GitHub의 검증된 이메일로 처음 가입하거나 처음 연동할 때 적용됩니다.",
  "通过 Google 已验证邮箱首次注册或首次绑定时应用。": "Google의 검증된 이메일로 처음 가입하거나 처음 연동할 때 적용됩니다.",
  "回调地址已写入并复制。": "콜백 주소가 입력되고 복사되었습니다.",
  "启用登录条款确认时，至少需要保留一份文档。": "로그인 약관 동의를 활성화할 때는 최소 한 개의 문서를 유지해야 합니다.",
  "登录条款文档名称不能为空。": "로그인 약관 문서 제목은 비워 둘 수 없습니다.",
  "公众号和移动应用不能同时启用。": "공식 계정과 모바일 앱은 동시에 활성화할 수 없습니다。"
};

function localText(zh: string, en: string, ko?: string): string {
  if (isZhLocale.value) {
    return zh;
  }
  if (isKoLocale.value) {
    return ko ?? koreanLocalTextMap[zh] ?? en;
  }
  return en;
}
...
function defaultLoginAgreementDocuments(): LoginAgreementDocument[] {
  return getDefaultLoginAgreementDocuments(locale.value);
}
...
/* ============ 系统设置 Tab 导航 ============ */
.settings-tabs-shell {
  @apply sticky z-20 -mx-1 rounded-2xl border border-white/80 bg-white/90 p-1.5 backdrop-blur-xl dark:border-dark-800 dark:bg-dark-950/95;
  top: 4.75rem;
  box-shadow:
