...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not render payment provider hints or documentation links", async () => {
    const wrapper = mountView();

    await flushPromises();
    await openPaymentTab(wrapper);

    expect(wrapper.text()).not.toContain("禁用服务商将同时禁用对应的实例");
    expect(wrapper.text()).not.toContain("查看支持的支付方式");
    expect(wrapper.text()).not.toContain("正在寻找合适的易支付服务商");
    expect(wrapper.find('a[href*="docs/PAYMENT"]').exists()).toBe(false);
  });
