<template>
  <div class="min-h-screen bg-gray-50 text-gray-900 dark:bg-dark-950 dark:text-white">
    ...
    <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:py-10">
      ...
      >
        <h1 class="text-lg font-semibold">{{ t('legal.loadFailedTitle') }}</h1>
        <p class="mt-2 text-sm">{{ t('legal.loadFailedDescription') }}</p>
      </section>
...
const siteName = computed(() => settings.value?.site_name || 'tokendeal')
...
const documentIcon = computed<LoginAgreementDocumentIcon>(() =>
  getLoginAgreementDocumentIcon(currentDocument.value?.title || '', 0)
)
