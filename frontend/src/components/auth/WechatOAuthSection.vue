...
function localizeWeChatHint(zh: string, en: string, ko: string): string {
  if (locale.value.startsWith('zh')) {
    return zh
  }
  if (locale.value.startsWith('ko')) {
    return ko
  }
  return en
}
  ...
  switch (resolvedStart.value.unavailableReason) {
    ...
    case 'native_app_required':
      return localizeWeChatHint(
        ...
        '현재 사이트에는 WeChat 모바일 앱 로그인만 구성되어 있습니다. 네이티브 앱에서 WeChat SDK를 통해 인증을 진행하세요.',
      )
    case 'not_configured':
