<template>
    ...
    <div class="flex items-start gap-2">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[13px] leading-5 text-gray-600 dark:text-dark-300">
          ...
          >
            {{ t('auth.agreement.checkboxPrefix') }}
          </label>
          <template v-for="(doc, index) in documents" :key="doc.id || doc.title">
            ...
            <span v-if="index < documents.length - 1">{{ agreementSeparator }}</span>
          </template>
        </p>
      </div>
    </div>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="font-medium">{{ t('auth.agreement.promptTitle') }}</p>
        <p class="mt-1 text-primary-700 dark:text-primary-200/80">
          {{ t('auth.agreement.promptDescription') }}
        </p>
      </div>
      <button
        type="button"
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
                  {{ t('auth.agreement.modalDescription', { date: updatedAt || t('auth.agreement.recently') }) }}
                </p>
              </div>
            </div>
          </div>
...
import { getLoginAgreementDocumentIcon, type LoginAgreementDocumentIcon } from '@/utils/loginAgreement'
...
const agreementSeparator = computed(() => locale.value.startsWith('zh') ? '、' : ', ')
...
function documentIcon(index: number, title: string): LoginAgreementDocumentIcon {
  return getLoginAgreementDocumentIcon(title, index)
}
</script>
